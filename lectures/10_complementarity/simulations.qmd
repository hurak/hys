...
```{julia}
#| code-fold: true
#| fig-cap: Simulation of a two-cart-spring mechanical system with a hard stop using a time-stepping numerical scheme. At first, both carts move towards each other unil their collision, after which they both accelerate towards the hard stop located at zero, the first cart a bit more, untill they collide again at the hard stop.
#| label: fig-time-stepping-for-mechanical-with-hard-stop
m1 = 1.0
m2 = 1.0
k1 = 1.0
k2 = 1.0
...
Plots.plot(t,X[1],label="x₁",lw=3,xlabel="t",ylabel="Positions",markershape=:circle,markersize=2)
Plots.plot!(t,X[2],label="x₂",lw=3,markershape=:circle,markersize=2)
```
...
\begin{aligned}
x_{k+1} &= x_k + h v_{k+1},\\
v_{k+1} &= v_k + \frac{h}{m} (F^+_{\mathrm{f},k} - F^-_{\mathrm{f},k} + F_{\mathrm{a},k}),
\end{aligned}
$$
...
$$\boxed{
\begin{aligned}
0 \leq v_k + \frac{h}{m} ({\color{blue}F^+_{\mathrm{f},k}} - {\color{blue}F^-_{\mathrm{f},k}} + F_{\mathrm{a},k}) + {\color{blue}\nu_{k+1}}\, &\perp\, {\color{blue}F^+_{\mathrm{f},k}} \geq 0,\\
0 \leq -\left(v_k + \frac{h}{m} ({\color{blue}F^+_{\mathrm{f},k}} - {\color{blue}F^-_{\mathrm{f},k}} + F_{\mathrm{a},k})\right) + {\color{blue}\nu_{k+1}}\, &\perp\, {\color{blue}F^-_{\mathrm{f},k}} \geq 0, \\
...
In the simulation below, we set the applied force to zero, $F_{\mathrm{a},k} = 0$, and we only simulate the response to some nonzero initial velocity. The simulation outcomes in @fig-time-stepping-for-coulomb-friction show that the object comes to a complete stop due to friction, as expected.
```{julia}
#| code-fold: true
#| fig-cap: Simulation of an object moving on a horizontal surface with Coulomb friction using a time-stepping numerical scheme in which the friction force is modelled using complementarity constraints. The object starts with an initial velocity and comes to a complete stop due to friction.
#| label: fig-time-stepping-for-coulomb-friction
using JuMP
using PATHSolver
using Plots
...
Modification of this example to model the situation of an object sliding down on and inclided plane is left as an exercise to the reader. Hint: the normal force that parametrizes the friction force will depend on the angle too, and the applied force $F_\mathrm{a}$ that we did not use here can be used to model the component of the gravity force that is parallel to the inclined plane. Obviously, the angle of the inclination and the friction coefficient must be such that the object that starts at some nonzero initial velocity eventually comes to a complete stop. Writing down such condition is a part of the exercise.
:::
