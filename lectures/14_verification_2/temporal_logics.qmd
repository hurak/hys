...
::: {#exm-progress-beverage-machine}
### Progress in a beverage vending machine
Recall our second example of a specified requirement for the beverage vending machine from the previous lecture. We required that "If the user pays, the machine will eventually dispenses a beverage". With the standard propositional logic we were unable to express this requirement. Note that the formula 
$$\text{paid} \implies \text{delivered}$$ 
is not the correct answer, because it would require that this implication holds for the same state, which is not what we require. Instead, we need to express that if the state trajectory contains a state where `paid` holds, then there must be some later state in the same trajectory where `delivered` holds. 
:::

::: {#exm-line}
### Insufficiency of propositional logic to express requirements on a traffic light controller
Consider the state automaton for a controller for two traffic lights (admittedly, we simplify here a lot). The state trajectory for each light is a sequence of the four color states $\{\text{green}$, $\text{yellow}$, $\text{red}$, $\text{red-yellow}\}$. We may want to impose a requirement such that $\text{green}$ is never on at both lights at the same time. This we can easily express just with the standard logical operators, namely, $\neg(\text{green}_1 \land \text{green}_2)$. But now consider that we require that sooner or later, $\text{green}$ must be on for each light (to guarantee *fairness*). And that this must be true all the time, that is, $\text{green}$ must come infinitely often. And, furthermore, that $\text{red}$ cannot come imediately after its respective $\text{green}$. 
:::
...
How do we express such property formula $\phi$? In order to be able to express requirements on future states, $\phi$ cannot be just a (proposional) logic formula, it must by a *Linear Temporal Logic* (LTL) formula. This is a general structure of the formula (using Backus-Naur form as used in computer science):
$$\boxed{
\phi \Coloneqq \text{true} \, | \, p \, | \, \neg \phi_1 \, | \, \phi_1 \land \phi_2 \, | \, \phi_1 \lor \phi_2 \, | \, \bigcirc \phi_1 \, | \, \Diamond \phi_1 \, | \, \Box \phi_1 | \, \phi_1 \sqcup \phi_2, \, \phi_1 \sqcap \phi_2,}
$$
where $p$ is an atomic formula, and $\phi_1$ and $\phi_2$ are some LTL formulas (apparently the above definition uses recursion).
...
We have mentioned at the beginning of this whole chapter on verification that a special kind of a requirement is that of a safety – something bad (say, a collision) must never happen. How do we express it using an LTL formula? The answer is provided by the first row above:
...
Finally, another type of a common requirement is that of stability – something good must eventually happen and then it must hold forever. How do we express it using an LTL formula? No surprise, the answer is also provided in the table above.
...
As we stated above, the formula $\forall \Diamond \Box \phi$ is not a valid CTL formula, it is a valid CTL* formula. But $\forall \Diamond \forall \Box \phi$ is a valid CTL formula. These two are not equivalent. Investigating the difference between the two shows that CTL* is indeed more expressive than CTL.
:::
...
Last but not least, STL is an extension of MITL towards continuous and hybrid systems. It not only supports the continuous time (as MTL/MITL does), but also it replaces the *propositional logic* with the *predicate logic* as it allows inequalities with real variables to play the role of atomic propositions. It is available for both continuous- and discrete-time systems. 
...
$$
\square (x(t) \geq 5.5 \Rightarrow \Diamond_{(1,10)} \mathrm{greenLight})
$$
$$
...
It turns out that if we only learn that the STL formula has been satisfied by a particular trajectory (aka signal), it is not clear whether the corresponding constraint has been satisfied with quite some margin or just so so. This is compensated for by introducing a *robustness degree*. See the literature for more.

### STL Monitoring, run-time verification

LTL and CTL logics (a bit less so CTL* and significantly less so MITL) lend themselves for some rigorous symbolic (fully exhaustive) verification (model checking) techniques with known complexities. In contrast, there are no such techniques for general STL formulas. Instead, verification of general STL formulas for the whole system must then be approached by evaluating them on individual trajectories started at initial states. This is called *monitoring*. The trajectories are obtained either by running simulations, experiments, or obtaining operational data. The outcomes from several trajectories are then processed statistically in Monte-Carlo style. This is known as *simulation-based* or *run-time verification*. 

### Time-varying control barrier functions for design of controllers from (a restricted set of) STL specifications

For some restricted classes of systems and STL formulas, verification techniques avoiding sampling the set of initial states exist. Those are based on *time-varying* barrier functions that we have introduced previously. In fact, upon introducing control into the concept of barriers, we get Control Barrier Functions (CBF), and time-varying CBFs can then be used to design (and not just analyze/verify) controllers satisfying a subset of STL formulas. See the latest book @lindemannFormalMethodsMultiAgent2025 (and papers by the same authors) for more details and references.