...
Furthermore, the ideas behind Petri nets even made it into international standards. Either directly or through the derived *GRAFCET* language, which in turn served as the basis for the *Sequential Function Chart* (SFC) language for PLC programming. See the [references](petri_nets_references.qmd).
...
![Example of a simple Petri net](petri_nets_figures/simplest_petri_net.png){#fig-simple-petri-net width=30%}
:::
...
![Example of a more complex Petri net](petri_nets_figures/complicated_petri_net.png){#fig-more-complex-petri-net width=70%}
:::
...
![Example of a marked Petri net](petri_nets_figures/simplest_petri_net_marked_1.png){width=30% #fig-simplest-petri-net-marked-1}
...
![Example of a marked Petri net with different marking](petri_nets_figures/simplest_petri_net_marked_2.png){width=30% #fig-simplest-petri-net-marked-2}
:::
...
![Example PN in the initial state at time 0, the transition $t_1$ enabled, but not yet fired](petri_nets_figures/transition_as_moving_tokens_1.png){width=50%}
...
![Transition $t_1$ in the example PN fired at time 1](petri_nets_figures/transition_as_moving_tokens_2.png){width=50%  #fig-example-PN-time-1}
...
::: {.callout-important}
## The number of tokens does not need to be preserved
In the example we can see for the first time, that the number of tokens does not need to be preserved.
:::

Now fire the $t_2$ transition

![Transition $t_2$ in the example PN fired at time 2](petri_nets_figures/transition_as_moving_tokens_3.png){width=50%}
...
![Transition $t_3$ in the example PN fired at time 1](petri_nets_figures/transition_as_moving_tokens_4.png){width=50%}
...
![Example of an unreachability in a Petri net](petri_nets_figures/unreachable_petri_net.png){width=30%}
...
![Example of a Petri net for reachability analysis](petri_nets_figures/petri_net_for_reachability_analysis.png){width=50%}

In @fig-reachability-tree we draw a *reachability tree* for this Petri net. 

![Reachability tree for an example Petri net](petri_nets_figures/reachability_tree.png){width=50% #fig-reachability-tree}

In @fig-reachability-graph we draw a reachability graph for this Petri net.

![Reachability graph for an example Petri net](petri_nets_figures/reachability_graph.png){width=50% #fig-reachability-graph}
:::

## Number of tokens does not need to be preserved
...
![](petri_nets_figures/and_convergence_and_divergence.png){width=600}

## OR-convergence and OR-divergence

![](petri_nets_figures/or_convergence_and_divergence.png){width=600}
...
![Nondeterminism in a Petri net](petri_nets_figures/petri_net_with_a_conflict.png){width=15%}
::: 
...
:::{#exm-event-graph}
## Event graph
![Example of an event graph](petri_nets_figures/event_graph.png){width=40%}
:::
...
:::{#exm-state-machine}
## State machine
![Example of a state machine](petri_nets_figures/state_machine_as_petri_net.png){width=45%}
:::
...
![Example of a Petri net](petri_nets_figures/transition_as_moving_tokens_1.png){width=50% #fig-state-equation-PN} 
...
![Queing systems and its components and transitions/events](petri_nets_figures/queueing_system_with_events.png){width=60% #fig-queueing-system-with-events}
...
![Petri net corresponding to the queing system](petri_nets_figures/petri_net_simple_queue.png){width=50%}
...
![Extended model of a queueing system](petri_nets_figures/petri_net_modified_queue.png){width=80% #fig-petri-net-modified-queue}
...
![Petri net for a beverage vending machine](petri_nets_figures/beverage_vending_machine_petri_net.png){width=80%}
:::
