...
\bm y(k) &= \mathbf C\bm x(k) + \mathbf D_u \bm u(k) + \mathbf D_\delta \bm \delta(k) + \mathbf D_z \bm z(k) + \mathbf D_0,\\
\mathbf E_\delta \bm \delta(k) &+ \mathbf E_z \bm z(k) \leq \mathbf E_u \bm u(k) + \mathbf E_x \bm x(k) + \mathbf E_0,  
\end{aligned}}
$${#eq-mld}
where
$$
...
$$
\begin{aligned}
\bm x(k+1) &= \mathbf A\bm x(k) + \mathbf B_u \bm u(k) + \mathbf B_w \bm w(k) + \mathbf B_0,\\
\bm y(k) &= \mathbf C\bm x(k) + \mathbf D_u \bm u(k) + \mathbf D_w \bm w(k) + \mathbf D_0,\\
\mathbf E_w \bm w(k) &\leq \mathbf E_u \bm u(k) + \mathbf E_x \bm x(k) + \mathbf E_0,  
\end{aligned}
$$
...
$$
x(k+1) = a_i x(k) + b_i u(k), \quad i\in\{1,2\}, 
$$
where $i=1$ if $x(k)< 0$ and $i=2$ otherwise. 

The auxiliary binary variable $\delta$ (actually $\delta(k)$, one for each discrete time $k$) is introduced to encode the switching condition:
$$
...
$$
but the somewhat abusive view of $q$ as both a logical (Boolean) and binary (integer) variable is a common practice.
:::
...
Accessed through Hybrid Toolbox for Matlab (see the [section on software for details on availability](software.qmd)), the HYSDEL parser can generate the MLD matrices automatically.
...
::: {#exm-mld-switched-no-matrices-yes-indicator-constraints}
Here we solve the problem from @exm-mld-switched but we do not formulate the matrices from @eq-mld explicitly. We use JuMP optimization modelling layer (for Julia language) for that. Note that JuMP also allows writing the indicators constraints directly, avoiding the need to introduce the big-M and small-m constants (but see the [section on software for details](software.qmd) for a discussion of caveats).
``` {julia}
#| code-fold: show
a₁ = -3/4
a₂ = 1/5
...
We plot the results in @fig-mld-switched-sim.
``` {julia}
#| fig-cap: Simulation of the switched linear system from @exm-mld-switched using its MLD model without explicitly formulating the MLD matrices
#| label: fig-mld-switched-sim
#| code-fold: true
xopt = value.(x)
uopt = value.(u)
δopt = value.(δ)
...
::: {#exm-heat-cool-mld-no-matrices-yes-indicator-constraints}
### Heating and cooling system modeled as MLD system without explicitly formulating the MLD matrices
The example is from the collection of demos for the Hybrid Toolbox for Matlab (see the [section on software for details on availability](software.qmd)). It is also described in the accompanying [presentation](https://cse.lab.imtlucca.it/~bemporad/hybrid/toolbox/hybrid_presentation.pdf), page 13. 
...
``` {julia}
#| fig-cap: Simulation of the heating-cooling system using its MLD model without explicitly formulating the MLD matrices
#| label: fig-heat-cool-mld-sim
#| code-fold: true
Topt₁ = value.(T1)
Topt₂ = value.(T2)
Toptₐ = value.(Ta)
