...
::: {#exm-higher-degree-polynomial-lf}
### Higher-degree polynomial Lyapunov function for @exm-no-cqlf
In this example we demonstrate that it is possible to combine the results from this chapter with those of the previous one. Namely, instead of searching for several quadratic Lyapunov-like functions that are then stitched together to form a piecewise quadratic Lyapunov function, we can also try to find a single higher-degree SOS polynomial function that qualifies as a Lyapunov function for both subsystems only on their respective domains.
...
```{julia}
#| code-fold: true
#| label: fig-higher-degree-polynomial-lf
#| fig-cap: State portrait with the higher-degree SOS polynomial Lyapunov function's sublevel sets superposed
x₁ = x₂ = range(-2.0, 2.0; length=100)
X₁ = [i for i in x₁, j in x₂]
X₂ = [j for i in x₁, j in x₂]
