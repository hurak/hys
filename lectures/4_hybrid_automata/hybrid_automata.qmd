...
### Semantics of transitions with external events
There does not appear to be a universal agreement on the semantics of transitions in hybrid automata with external events. These are two common possibilities:

1. The transition occurs when the external event arrives *and* the guard condition is satisfied (the invariant set is relevant as before).
2. The transition occurs as soon as the corresponding external event occurs (the guard condition is assumed to be automatically satisfied).
