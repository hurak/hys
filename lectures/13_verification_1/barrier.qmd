...
$$
\dot{\bm x}(t) = \mathbf f(\bm x). 
$$

We now define two regions of the state space $\mathcal X$: 

- the set of initial states $\mathcal X_0$, 
- and the set of unsafe states $\mathcal X_\mathrm{u}$. 

Our goal is to prove (certify) that the system does not reach the unsafe states for an arbitrary initial state $\bm x(0)\in \mathcal X_0$.
...
$$\nabla B(\bm x)^\top \mathbf f(\bm x) \leq 0\quad \forall \bm x\; \text{such that} \; B(\bm x) = 0.$$

Now, upon finding a function $B(\bm x)$ with such properties, we prove (certify) safety of the system. Indeed, the (existence of the) function serves as a certificate of safety.
...
$$\nabla B(\bm x)^\top \mathbf f(\bm x) \leq 0\quad \forall \bm x\in \mathcal X.$$
...
\begin{aligned}
\dot x_1 &= x_2,\\
\dot x_2 &= -x_1 + \frac{1}{3}x_1^3 - x_2.
\end{aligned}
$$ 
...
The optimization problem is feasible, a barrier function is found. We can visualize the vector field, the initial and unsafe sets, and the boundary function level set, along with a few simulated trajectories, as in @fig-barrier.
``` {julia}
#| code-fold: true
...
$$\nabla B_i(\bm x)^\top \mathbf f_i(\bm x) \leq 0\quad \forall \bm x\;\text{such that} \; B_i(\bm x) = 0,$$

$$
\begin{aligned}
B_i(\bm x) \leq 0\quad &\forall \bm x \in \mathcal R(q_j,q_i,\bm x^-)\,\text{for some}\, q_j\; \text{and}\; \bm x^-\in\mathcal G(q_j,q_i)\,\text{with}\, B_j(\bm x^-)\leq 0.
\end{aligned}
$$

## Convex relaxation of barrier certificates for hybrid systems

$$\nabla B_i(\bm x)^\top \mathbf f_i(\bm x) \leq 0\quad\; \forall \bm x\in \mathcal X_0(q_i),$$

$$
\begin{aligned}
B_i(\bm x) \leq 0\quad &\forall (\bm x, \bm x^-)\,\text{such that}\, \bm x \in \mathcal R(q_j,q_i,\bm x^-) \;\text{and}\; \bm x^-\in\mathcal G(q_i,q_j).
\end{aligned}
$$