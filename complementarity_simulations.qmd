...
We now have enough to start implementing a solver for this system, provided we have an access to a solver the MCP (see the [page dedicated to software for complementarity problems](complementarity_software.qmd)). But before we do it, we analyze the problem a bit deeper. In this particular small and simple case, it is still doable with just a pencil and paper.
...
Let's now apply the time-stepping method for a mechanical system with a hard stop as described in the Example 2 in the [section on complementarity systems](complementarity_systems.qmd). We start by formulating the *semi-explicit* Euler scheme
...
![Coulomb friction to be modelled using complementarity constraints](complementarity_figures/coulomb_friction.png){width=40% #fig-coulomb-friction}
