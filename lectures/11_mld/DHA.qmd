...
Is it possible? Unfortunately no. At least not exactly in this form. But something close to it is achievable instead. 
...
Their interconnection is shown in @fig-dha.

![Discrete-time hybrid automaton](figures/dha.png){#fig-dha width=90%}
...
$$
\begin{aligned}
\bm x_\mathrm{c}(k+1) &= \mathbf A_{i(k)} \bm x_\mathrm{c}(k) + \mathbf B_{i(k)} \bm u_\mathrm{c}(k) + \mathbf f_{i(k)},\\
\bm y_\mathrm{c}(k) &= \mathbf C_{i(k)} \bm x_\mathrm{c}(k) + \mathbf D_{i(k)} \bm u_\mathrm{c}(k) + \mathbf g_{i(k)}.
\end{aligned}
$$
...
$$
\mathbf H_i \bm x_\mathrm{c}(k) + \mathbf J_i \bm u_\mathrm{c}(k) + \mathbf K_i \leq 0.
$$

The event indicated by the vector binary variable (through so-called one-hot encoding)
$$
\bm \delta_e(k) = \mathbf h(\bm x_\mathrm{c}(k), \bm u_\mathrm{c}(k)) \in \{0,1\}^{n_e}, 
$$
where
$$
h_i(\bm x_\mathrm{c}(k), \bm u_\mathrm{c}(k)) = \begin{cases}1 & \mathbf H_i \bm x_\mathrm{c}(k) + \mathbf J_i \bm u_\mathrm{c}(k) + \mathbf K_i \leq 0,\\ 0 & \text{otherwise}. \end{cases}
$$

### Finite state machine (FSM)

$$
\bm x_\mathrm{d}(k+1) = \mathbf f_\mathrm{d}(\bm x_\mathrm{d}(k),\bm u_\mathrm{d}(k),\bm \delta_e(k)).
$$
...
$$
i(k) = \mu(\bm x_\mathrm{d}(k), \bm u_\mathrm{d}(k), \bm \delta_e(k)).
$$
...
$$
\begin{aligned}
\bm \delta_e(k) &= \mathbf h(\bm x_\mathrm{c}(k), \bm u_\mathrm{c}(k))\\
i(k) &= \mu(\bm x_\mathrm{d}(k), \bm u_\mathrm{d}(k), \bm \delta_e(k))\\
\bm y_\mathrm{c}(k) &= \mathbf C_{i(k)} \bm x_\mathrm{c}(k) + \mathbf D_{i(k)} \bm u_\mathrm{c}(k) + \mathbf g_{i(k)}\\
\bm y_\mathrm{d}(k) &= \mathbf g_\mathrm{d}(\bm x_\mathrm{d}(k), \bm u_\mathrm{d}(k), \bm \delta_e(k))\\
\bm x_\mathrm{c}(k+1) &= \mathbf A_{i(k)} \bm x_\mathrm{c}(k) + \mathbf B_{i(k)} \bm u_\mathrm{c}(k) + \mathbf f_{i(k)}\\
\bm x_\mathrm{d}(k+1) &= \mathbf f_\mathrm{d}(\bm x_\mathrm{d}(k),\bm u_\mathrm{d}(k),\bm \delta_e(k)).
\end{aligned}
$$
