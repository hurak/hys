...
Here we solve the problem from @exm-mld-switched but we do not formulate the matrices from @eq-mld explicitly. Instead, we rely on the functionality provided by modern optimization modelling languages (here we use JuMP for Julia) to formulate the problem directly by collecting all the constraints. 

We were also able to avoid the need to introduce the big-M and small-m constants, as JuMP makes it possible to write the indicator constraints directly (but see the discussion in the [section on software for details](software.qmd)).
...
using Plots
p1 = plot(0:N, xopt, xlims=(-0.1, 10.1), label="x(k)", xlabel="k", ylabel="x(k)", markershape=:circle, markersize=2)
p2 = plot(0:N-1, uopt, xlims=(-0.1, 10.1), label="u(k)", xlabel="k", ylabel="u(k)", markershape=:circle, markersize=2)    
p3 = plot(0:N-1, δopt, xlims=(-0.1, 10.1), label="δ(k)", xlabel="k", ylabel="δ(k)", markershape=:circle, markersize=2)
